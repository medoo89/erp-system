...
<body>
@php
    $days = $salarySlip->days->sortBy(fn ($day) => $day->work_date ?? $day->date ?? $day->id);
...
<div class="page">
    ...
    <form method="POST" action="{{ route('admin.salary-slips.attendance.update', $salarySlip) }}">
        @csrf
        @method('PUT')
        ...
        <div class="card">
                    ...
                    @foreach($days as $day)
                        @php
                            ...
                            if (in_array($status, ['absent', 'unpaid_leave'], true)) {
                                ...
                                $isPaid = isset($day->is_paid) ? (bool) $day->is_paid : (isset($day->paid) ? (bool) $day->paid : true);
                            }
            ...
            <div class="actions">
                ...
                <a href="{{ route('filament.admin.resources.salary-slips.view', ['record' => $salarySlip]) }}" class="btn btn-secondary">Cancel</a>
            </div>
        </div>
    </form>
</div>
