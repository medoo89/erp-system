...
<div class="page-shell">
    <div class="hero">
        ...
        <div class="actions">
            <a href="{{ route('admin.projects.profile', ['project' => $project]) }}" class="btn btn-primary">Back to Project</a>
            @if($project->client)
                <a href="{{ route('admin.clients.profile', ['client' => $project->client]) }}" class="btn btn-secondary">Back to Client</a>
            @endif
        </div>
    </div>
            ...
            <table>
                    ...
                    <tbody>
                        @forelse($items as $item)
                            <tr>
                                ...
                                <td>{{ $item->client_name ?? $project->client?->name ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="empty">No invoice records found for this project.</td></tr>
                        @endforelse
                    </tbody>
                @elseif($type === 'salary_slips')
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="5" class="empty">No salary slip records found for this project.</td></tr>
                        @endforelse
                    </tbody>
                @else
                    <thead>
                    ...
                    <tbody>
                        @forelse($items as $item)
                            <tr>
                                ...
                                <td>{{ optional($item->expense_date ?? $item->created_at)->format('M j, Y') ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="empty">No expense records found for this project.</td></tr>
                        @endforelse
                    </tbody>
                @endif
            </table>
