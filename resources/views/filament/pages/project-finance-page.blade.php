<x-filament-panels::page>
    ...
    <div class="sf-wrap">
        <div class="sf-hero">
            ...
            <div class="sf-sub">Project: {{ $project->name ?? '-' }} • Client: {{ $client?->name ?? '-' }}</div>
                ...
                <table>
                    ...
                    @elseif($type === 'salary_slips')
                        ...
                        @forelse($rows as $row)
                            <tr>
                                <td>{{ $row['employee'] }}</td>
                                <td>{{ $row['total_slips'] }}</td>
                                <td>{{ $row['latest_status'] }}</td>
                                <td>{{ $row['latest_period'] }}</td>
                                <td>{{ $row['latest_amount'] }}</td>
                                <td>{{ $row['total_amount'] }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="empty">No salary slip records found for this project.</td></tr>
                        @endforelse
                        </tbody>
                    @else
                        <thead>
